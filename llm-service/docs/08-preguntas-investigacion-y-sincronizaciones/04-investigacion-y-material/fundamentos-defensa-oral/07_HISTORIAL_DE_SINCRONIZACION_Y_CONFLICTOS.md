...
> **Aplicación actual.** Esta comparación didáctica se subordina a [00](../../../00-gobierno-y-evolucion/01-fuentes-de-verdad-y-convenciones.md): `llm-service`, Gateway/Eureka, Kafka para eventos y alcance MVP definido por el PRD.
...
Pusheado — commit `4d30a1f`, con [14-sincronizacion-guia-didactica.md](../../../08-preguntas-investigacion-y-sincronizaciones/02-sincronizacion-con-guia-didactica.md).
...
| **Nosotros** | Java Spring Boot, porque Programación IV es materia de Java y la integración con Spring Cloud pesa más ([02](../../../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md), Parte 2) |
...
> figura además entre las siete revisadas de [08](../../../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md), fila 3—, el
...
**A ajustar en nuestros docs:** [05 · Seguridad](../../../04-seguridad-datos-y-cumplimiento/01-seguridad-y-guardarrailes.md) y el ADR-009 de
[08](../../../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md) dicen "sin streaming en el MVP". **Hay que revisarlo.**
...
**Resolución: prevalece nuestro catálogo** ([03](../../../03-capacidades-de-ia/01-modelos-costos-y-contexto.md)), que tiene precios
...
([02](../../../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md), Parte 1).
...
| **Nosotros** | A 120 usuarios probablemente **no haga falta**: Postgres con `FOR UPDATE SKIP LOCKED` + caché en memoria alcanza ([06](../../../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md), Parte 6) |
...
|---|---|---|---|
| **A-1** | **Buffer Interceptor en streaming** con máquina de estados | Resuelve lo que declaramos bloqueante | [05](../../../04-seguridad-datos-y-cumplimiento/01-seguridad-y-guardarrailes.md) |
| **A-2** | 🔴 **Hiperparámetros por rol**: temperatura, top-p, top-k, **seed** | **`temperature: 0` y `seed` fijo en el evaluador atacan directamente el problema de reproducibilidad** que marcamos como debilidad de los LLM para poner notas | [13](../../../03-capacidades-de-ia/03-rubricas-y-prompts.md) |
| **A-3** | **Timeouts concretos por rol** (moderador 1 s, tutor 45 s, evaluador 120 s) | Nosotros solo dimos objetivos de latencia, no timeouts | [06](../../../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) |
| **A-4** | **DDL completo de PostgreSQL** con `UUID`, `JSONB` e índices GIN | Nosotros describimos las tablas, ellos las escribieron | [12](../../../03-capacidades-de-ia/rag-e-ingesta/01-almacenamiento-e-ingesta.md) |
| **A-5** | 🔴 **Triggers `BEFORE UPDATE OR DELETE`** para forzar inmutabilidad de notas | **Es mejor que nuestra regla "append-only por convención"**: lo hace cumplir la base, no la disciplina del equipo | [12](../../../03-capacidades-de-ia/rag-e-ingesta/01-almacenamiento-e-ingesta.md) |
| **A-6** | **`tokens_usage_ledger`** — tabla de consumo por usuario y día | Nosotros dijimos "contadores en Redis"; una tabla es auditable y sobrevive reinicios | [12](../../../03-capacidades-de-ia/rag-e-ingesta/01-almacenamiento-e-ingesta.md) |
| **A-7** | **PII scrubber con regex** antes de mandar al proveedor | Nosotros dijimos "no mandes PII"; ellos lo implementan como capa | [05](../../../04-seguridad-datos-y-cumplimiento/01-seguridad-y-guardarrailes.md) |
| **A-8** | **Arquitectura interna Onion / Clean** | Nosotros dimos 8 módulos sin estructura de capas adentro | [02](../../../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) |
| **A-9** | **Calibración nocturna programada** (Celery Beat) | Nosotros dijimos "mensual" (PAR-15); ellos la automatizan | [04](../../../03-capacidades-de-ia/02-funciones-de-ia.md) |
