# Correcciones aplicadas — las tres presentaciones revisadas

`mapa-conceptual-interactivo.html`, `guia-golden-set.html` e
`informe-gestion-modelos.html` se armaron sobre una versión anterior de la documentación, no
sobre la vigente. Eso las dejó desalineadas en ocho puntos.
...
> **Estas tres ya no son idénticas a su versión original.** Fue una decisión explícita:
> se privilegió que digan lo mismo que la documentación vigente antes que conservarlas
> intactas.
...
`docs/17` (histórico, retirado) fija el contrato bajo `/ai/…`.
...
Cada requerimiento y cada plan tenía un `repoPath` con la estructura de una versión anterior del
repositorio, y el botón *«Ver en GitHub»* apuntaba a esa versión.

**Aplicado:** los `repoPath` apuntan a rutas que existen en el repositorio vigente, y el botón
también. Se verificó archivo por archivo que los destinos existen.
...
**No venía así originalmente.** La clave se perdió en el commit que trajo las presentaciones a
este repositorio, junto con los cambios de endpoints, Java y ADR. No se notaba sin abrir la
consola del navegador. Corregido y verificado: los 36 requerimientos, los 15 ADR y los seis
glosarios cargan.
