# Plantilla — Épica (Wiki / backlog de Taiga)

> Template oficial de Épica de la Wiki de Taiga «Plataforma de Aprendizaje
> Gamificado de Programación». Uso **obligatorio** para uniformidad
> (ver [27 · Guía de la Wiki §7](../07-guia-wiki-taiga.md)).
>
> Copiar este contenido al crear una épica nueva en el backlog. Reemplazar `GXX` por
> el número de grupo asignado por la cátedra y borrar los textos de ejemplo antes de
> publicar.
>
> El catálogo de épicas del equipo (EP-01…EP-10, con pareja líder, sprints donde aporta,
> fase y requisitos que cubre) está en
> [`35-backlog-ejecutable.md`](../04-backlog-ejecutable.md)
> y en el índice [`epicas/README.md`](../09-epicas-historias-tareas-sprints/epicas/README.md); esta plantilla es el
> **formato de presentación** de cada una en Taiga y **no** repite esos datos (sin bloque
> `| Campo | Valor |`).
>
> Las épicas no se estiman ni se comprometen: se cierran cuando todas sus historias pasan
> la DoD ([23 · §9.2](../03-plan-de-construccion-del-producto.md)). **No llevan prioridad
> MoSCoW, ni checklist INVEST, ni puntos Fibonacci** — son criterios de Historia de
> Usuario ([29 · §3 y §6](../08-guia-de-historias-de-usuario.md)). La prioridad de
> una épica es su **fase (F1/F2/F3) + sprint**, y vive en el catálogo.

---

# GXX — [TÍTULO DEL ÉPICO]

---

## Objetivo

[Describir en 1–2 líneas qué valor de negocio / usuario entrega el épico.]

---

## Suposiciones y Restricciones

- **Suposiciones:** [listar]
- **Restricciones (legales / técnicas):** [listar]

---

## Criterios de Aceptación a nivel Épico

- [ ] El conjunto mínimo de historias permite [flujo extremo a extremo].
- [ ] KPIs iniciales alcanzan: [valores].
- [ ] Sin regresiones críticas en [áreas / sistemas].
- [ ] Observabilidad y alertas configuradas (logs, métricas, traces).
- [ ] Documentación de uso y operación publicada.

---

## Dependencias / Impactos

- **Servicios / APIs:** [listar]
- **Módulos afectados:** [listar]
- **Otros equipos:** [listar]
- **Impacto en datos / migraciones:** [detallar]
- **Feature toggles / flags:** [sí / no, plan de retiro]
