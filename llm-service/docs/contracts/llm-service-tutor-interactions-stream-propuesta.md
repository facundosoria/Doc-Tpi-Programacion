# Propuesta — Tutor en streaming SSE (Buffer Interceptor)

> **Estado: propuesto, no fusionado al contrato ejecutable.** No es material histórico — es una
> especificación lista para fusionar el día que se decida activar streaming en el tutor.

Esta propuesta completa el contrato vigente con la variante en streaming del tutor: la respuesta
se emite token a token en vez de un bloque único. Implementa el **Buffer Interceptor** de la guía
didáctica ([08 · sincronización con la guía didáctica](../08-preguntas-investigacion-y-sincronizaciones/02-sincronizacion-con-guia-didactica.md)), que revisa a
[ADR-009](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md).

**Debe fusionarse en [`llm-service.openapi.yaml`](llm-service.openapi.yaml) cuando se decida
"streaming: propagar o revertir" (ítem I-10 — decisión interna de Tema 07, no requiere sesión
conjunta) y `practice-service` (Tema 05) acuerde consumir SSE.** Hasta entonces el contrato
ejecutable sigue siendo solo `POST /api/llm/tutor/interactions` con respuesta completa. Al
fusionar, `info.version` pasa a la siguiente versión menor (cambio aditivo, retrocompatible).
