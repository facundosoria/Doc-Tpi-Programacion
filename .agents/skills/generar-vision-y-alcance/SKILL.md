...
description: >-
  ...
  Es la capa 1 del pipeline: su salida alimenta a contratos-api-gateway, contratos-kafka y a
...
Se **encadena** hacia abajo: `contratos-api-gateway` (HTTP) y `contratos-kafka` (eventos) toman las **funciones** y las
...
3. **Funciones / capacidades.** Una fila por función: nombre, qué hace, sync/async si
   ...
   `contratos-api-gateway` / `contratos-kafka` y del catálogo de épicas.
