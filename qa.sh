...
# Tu mail de git. Se resuelve ACA, en el host, porque adentro del contenedor el
# `git config` global no existe: se monta el repo, no tu HOME. Sin esto la etapa
# `formato` no puede distinguir tu commit del de un companero, y para no
# arriesgarse a reescribir codigo ajeno se limita a lo que tengas sin commitear.
QA_AUTOR="${QA_AUTOR:-$(git config user.email 2>/dev/null || true)}"
...
if [ "$REMOTO" = "1" ]; then
  ...
  QA_CODIGO=0
  ssh -p "$QA_PUERTO" "$QA_REMOTO" "cd '$QA_DESTINO' && CI=1 QA_BASE='${QA_BASE:-}' QA_PREFIJO='${QA_PREFIJO:-}' QA_AUTOR='${QA_AUTOR:-}' QA_M2_VOLUME='tpi-qa-m2-remoto-$QA_USUARIO' QA_SPOOL='$QA_BUZON_REMOTO' QA_ORIGEN=remoto QA_USUARIO='$QA_USUARIO' bash qa.sh$QA_ARGS" || QA_CODIGO=$?
