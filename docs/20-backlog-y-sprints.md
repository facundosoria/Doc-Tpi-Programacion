...
|---|---|---|
| **1** | Una tarde sin sorpresas | Rotar una API key, sumar un chequeo al gate |
...
correspondiente más lo que antes quedaba sin dueño: las pantallas, los contratos y el gate.
...
3. **`repository/` y `entity/` son territorio compartido y el punto de conflicto más probable**
   ...
   la migración de otro y se descubre en el CI, no en tu máquina.
...
| E05-09 | Pruebas de algo no determinístico | Snapshots con tolerancia por dimensión, según [06](06-operacion-e-ingenieria.md). Cambiar un prompt sin correr esto queda prohibido por el gate | 5 | E05-03 | 🟡 |
...
**Célula C1.** El gate ya existe ([16](16-pipeline-y-verificaciones.md)); esta épica es hacerlo
cumplir sobre el código nuevo y sumarle lo que hoy no mira.

| ID | Historia | Criterio de aceptación | SP | Dep. | Prio |
|---|---|---|---|---|---|
| E11-01 | El gate en verde sobre el micro, en cada PR | `qa.sh` pasa sobre `ms-evaluacion-llm` y ningún PR mergea en rojo | 3 | — | 🔴 |
...
| E11-03 | Las corridas visibles en el panel del CI | El panel de `tools/ci-front` muestra las corridas del micro, no solo las de documentación | 3 | E11-01 | 🟢 |
...
- El gate corre sobre el micro en cada PR.
...
1. **El gate pasa en verde** ([16](16-pipeline-y-verificaciones.md)): formato, cobertura, PMD,
   enlaces, ortografía y secretos.
