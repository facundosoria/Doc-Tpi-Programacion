# Plantilla — Registro de sprint

Copiar para cada sprint y completar durante Planning, ejecución y cierre. Los valores de
referencia no acreditan asistencia, horas reales ni funcionalidades terminadas. No marcar
una casilla sin su evidencia.

> Parámetros del equipo (fijar una vez y reusar): `N` = integrantes ·
> `HS` = h-persona/semana declaradas · `L` = semanas por sprint ·
> nominal del sprint = `HS × L`.
...
Las horas disponibles se toman de la planilla de disponibilidad del equipo (`N`
integrantes) e incluyen reuniones.
...
> El **trabajo estimado de las recetas** es un piso, no un tope. La diferencia con la
> capacidad comprometible es margen para re-estimar en Planning e imprevistos.
...
La demo pertenece a la Review. La coordinación de dependencias no es reunión aparte: va
dentro de la sincronización.

| Reunión | Cantidad | Minutos por sesión | Asistentes internos | Referencia horas-persona | Fecha(s) | Horas-persona reales |
|---|---:|---:|---:|---:|---|---:|
| Planning | | | | | | |
| Daily / sincronización (incl. dependencias) | | | | | | |
| Review con demo | | | | | | |
| Retrospectiva | | | | | | |
| Refinamiento | | | | | | |
| **Total programado** | | | | | | |

Reuniones extraordinarias, preparación, actas y coordinación asincrónica se registran en
la reserva. Si una reunión planificada se extiende, la diferencia consume reserva.
...
IDs históricos no se reutilizan. Una historia reprogramada conserva su ID y cambia de
sprint objetivo. La estimación incluye backend, interfaz, pruebas, review de código,
integración, documentación y despliegue.
