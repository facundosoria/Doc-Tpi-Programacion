...
flowchart TB
    ...
    subgraph nuestro["TEMA 07 · ms-evaluacion-llm — NUESTRO"]
        API["M8 · la API<br/>6 endpoints"]
        ...
        Q[("Cola interna<br/>tecnologia SIN DECIDIR")]
        API --> AIGW
        API --> Q
        Q --> WK
        WK --> AIGW
    ...
    BUS["BUS DE EVENTOS<br/>contrato del Tema 11"]

    T02["Tema 02 · Cursos"]
    T03["Tema 03 · Motor de desafios"]
    T05["Tema 05 · Desafios practicos"]
    T11["Tema 11 · Chat"]
    T12["Tema 12 · Backoffice"]

    FE --> NGINX
    NGINX --> GW
    T02 -->|"esta aprobada la calibracion?"| GW
    T03 -->|"evaluar · corregir"| GW
    T05 -->|"tutor · corregir"| GW
    T11 -->|"moderar un mensaje"| GW
    T12 -->|"estado, deriva, costo"| GW
    GW --> API
    AIGW --> LLM
    nuestro -->|"score listo · calibracion · incidente"| BUS
    BUS -->|"intento_cerrado · curso_archivado"| nuestro
...
## 2. Los seis endpoints: verbo, ruta y quién los llama
